...
class SSA():
    def __init__(self, func, n_dim=None, pop_size=20, max_iter=50, verbose=False,search_space=None):
        self.func = func
        ...
        # 设置搜索空间
        if search_space and len(search_space) == n_dim:
            lb = [search_space[i][0] for i in range(n_dim)]
            ub = [search_space[i][1] for i in range(n_dim)]
        else:
            lb = [-512] * n_dim
            ub = [512] * n_dim
...
if __name__ == '__main__':
    ...
    pop_size = 100
    max_iter = 100
    ssa = SSA(demo_func, n_dim=n_dim, pop_size=pop_size, max_iter=max_iter, search_space=search_space)
    ssa.run()
